...
Read [README.md](README.md) and [.agents/skills/native-popovers/SKILL.md](.agents/skills/native-popovers/SKILL.md) before changing the tooltip.
...
- Keep the public names `TooltipProvider`, `TooltipAnchor`, `Tooltip`, and `TooltipArrow`. They all live in `src/tooltip/tooltip.tsx`, split in regions. Do not add an index barrel; `package.json` exports each module path.
...
- Only `Tooltip` and `TooltipArrow` may subscribe to the controller. The anchor must not render on hover or focus.
